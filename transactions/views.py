...
from transactions.constants import DEPOSIT, WITHDRAWAL,LOAN, LOAN_PAID, SEND, RECIEVE
...
class WithdrawMoneyView(TransactionCreateMixin):
    ...
    def form_valid(self, form):
        # print(self.request.user.account.is_bankrupt)
        if self.request.user.account.is_bankrupt:
            messages.error(self.request, f'Sorry, the bank is bankrupted. Withdrawal is not allowed.')
    ...
    def get(self, request, loan_id):
        ...
        if loan.loan_approve:
            ...
            if loan.amount < user_account.balance:
                ...
                loan.save()
                send_transaction_email(self.request.user, loan.amount, "Loan Paid Message", "loan_pay_email.html") 
...
class TransferView(LoginRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        reciever_account = UserBankAccount.objects.filter(account_no=account_number).first()
        
        if reciever_account is None:
            messages.error(self.request, f'Account with number {account_number} not found.')
            return redirect('transaction_report')
        
        if amount > sender_account.balance:
            messages.error(self.request,f'Insufficient balance to transfer')
            return redirect('transfer_money')
        
        reciever_account.balance += amount
        sender_account.balance -= amount
        sender_account.save(update_fields=['balance'])
        reciever_account.save(update_fields=['balance'])
        messages.success(self.request, f'$ {amount} is transfered successfully to {reciever_account}')
        send_transaction_email(sender_account.user, amount, "Transfer Money Message", "send_money_email.html")
        send_transaction_email(reciever_account.user, amount, "Recieve Money Message", "recieve_money_email.html")
